...
(function ($) {
    ...
    var rtsJs = {
        ...
        /** Home hero: crossfade campus slides + dots (see index.php data-sms-hero-slideshow) */
        smsHeroSlideshow: function () {
            $(document).ready(function () {
                var root = document.querySelector('[data-sms-hero-slideshow]');
                if (!root) return;

                var slides = root.querySelectorAll('.sms-hero-cu__slide');
                var dots = root.querySelectorAll('.sms-hero-cu__slide-dot');
                var n = slides.length;
                if (n === 0) return;
                ...
                dots.forEach(function (dot) {
                    dot.addEventListener('click', function () {
                        var idx = parseInt(dot.getAttribute('data-sms-hero-go'), 10);
                        if (!isNaN(idx)) {
                            go(idx);
                            start();
                        }
        ...
        stickyHeader: function (e) {
			function sticky_header() {
				/* SMS branded header: use CSS position:sticky only — template .fixed removes header
				   from document flow and leaves an empty band, which looks like a “double header”. */
				const headerSticky = $('header.header__sticky').not('.sms-header-pro');
